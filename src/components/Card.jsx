...
function Card({card : {id, title, content}}) {
    ...
    function onClickEditNote() {
        document.querySelector(".edit-note-menu").classList.remove("invisible");
        document.querySelector(".edit-note-menu input").value = title;
        document.querySelector(".edit-note-menu textarea").value = content;
        document.querySelector(".edit-note-menu button:last-child").dataset.id = id;
    }
    ...
    return (
        <div className="group relative p-6 bg-white dark:bg-zinc-800 rounded-lg shadow-md transition-all hover:-translate-y-1 hover:shadow-xl cursor-pointer">
            <div className="absolute top-3 right-3 flex gap-2">
                <button className="p-1.5 hover:bg-gray-100 dark:hover:bg-gray-700 rounded-full transition-colors" onClick={onClickEditNote}>
